...
namespace mongo {
    ...
    Security::Security() {
        static int n;
        assert(++n == 1); //massert( 10352 , "Security is a singleton class", ++n == 1);
        init();
    }
    ...
    void Security::init() {
        ...
        if ( !_devrandom->is_open() )
            assert(false); //massert( 10353 , std::string("can't open dev/urandom: ") + strerror(errno), 0 );
#elif defined(_WIN32)
...
/*
        if ( do_md5_test() )
            assert(false); // massert( 10354 , "md5 unit test fails", false);
*/
#endif
    }
    ...
    nonce64 Security::__getNonce() { 
        ...
        assert(!_devrandom->fail()); // massert(10355 , "devrandom failed", !_devrandom->fail());
#elif defined(_WIN32)
        unsigned a=0, b=0;
        verify( rand_s(&a) == 0 );
        verify( rand_s(&b) == 0 );
    ...
    boost::mutex nonceMutex;
    nonce64 Security::_getNonce() {
        boost::mutex::scoped_lock lk(nonceMutex);
        if( !_initialized )
            init();
        return __getNonce();
    }
